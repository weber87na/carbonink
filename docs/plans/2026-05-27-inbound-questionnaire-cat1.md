...
- [ ] **Step 1: Write the migration** following the spec's table-recreate pattern. Order:
  1. Begin transaction
...
- [ ] **Step 2: Write migration test** — using the standard `tests/helpers/test-db.ts` helper, seed one outbound questionnaire pre-migration, run migrations, assert direction='outbound', customer.role='customer', question.tier IS NULL, activity_data.inbound_* IS NULL on existing rows.

- [ ] **Step 3: Update shared types** — add all the types from the "Types" section above. Update `Questionnaire`/`Customer`/`Question`/`ActivityData` to include new columns.

- [ ] **Step 4: Service shims** — `customer-service` gets:
...
- [ ] **Step 1: Write `cat1.ts`** following the spec exactly. 7 questions (3 metadata, 1 Tier 1, 3 Tier 2). Each carries `position`, `tier`, `kind`, `raw_zh`, `raw_en`, `expected_unit`, `cell_ref`.
- [ ] **Step 2: Write `index.ts`** — registry pattern. Single function `getTemplate(kind)`; future templates just need to be added to the switch. Throws on unknown kind.
- [ ] **Step 3: Write template integrity tests**:
...
- [ ] **Step 1: Write test** — happy path, supplier-not-found, period-not-found, unknown template_kind, subset-of-positions inclusion
- [ ] **Step 2: Implement service** (this method only — others stubbed with `throw new Error('Task N')`)
...
- [ ] **Step 1: Write test** — render the Cat 1 template, parse the returned Buffer back into ExcelJS, assert:
...
- [ ] **Step 2: Implement renderer**. Helper functions: `buildCoverSheet`, `buildQuestionSheet`, `buildSentinelSheet`.
- [ ] **Step 3: Verify + commit**
...
- [ ] **Step 1: Tests** — round-trip a synthetic answered workbook through render → fill cells → parse → assert answers match. Plus: each warning kind has its own test.
...
- [ ] Test: happy path → buffer returned, status flipped, one audit row
- [ ] Test: re-export from status='sent' → buffer returned, no second audit row
- [ ] Test: trying to export an outbound questionnaire → throws
- [ ] Commit
...
- [ ] Tests: happy Tier 2 path, happy Tier 1 path, sentinel mismatch (throws), period mismatch (warning surfaced), blank workbook (warning + tier_selected=null), re-import overwrites tentative answers
- [ ] Tests for `getIngestPreview` — idempotent
- [ ] Commit
...
- [ ] Tests: Tier 2 happy, Tier 1 happy (with quantity), Tier 1 missing quantity (throws), idempotent re-ingest, partial-acceptance soft fail
- [ ] Test: sentinel EF row is created exactly once per (supplier, year)
- [ ] Test: emission_source is reused on second ingest (same supplier × year)
- [ ] Commit
...
- [ ] Tests: each channel happy path; answer:generate refuses inbound questionnaires
- [ ] Commit
...
- [ ] No unit tests for routes themselves (consistent with v1); test the components via shallow render + interaction tests if time permits
- [ ] Verify route lazy-loading works (TanStack file-based routing picks up the new file)
- [ ] Commit
...
- [ ] **Step 1: Add keys to both files** (alphabetical order within both)
- [ ] **Step 2: Run `pnpm --filter carbonink test -- --run tests/i18n`** (or the equivalent — find the alignment test)
- [ ] **Step 3: Wire keys into the components from T10**
- [ ] **Step 4: Commit**
...
### Task 13: Manual smoke (USER ACTION)

Per the spec's "Smoke checklist". User-side actions only — no code in this task.
...
- All 12 implementer tasks committed
- `pnpm --filter carbonink test -- --run` passes (baseline 830 → expected ≥860)
- `pnpm --filter carbonink typecheck` clean
- `pnpm --filter carbonink exec biome check <changed files>` clean (existing repo-wide debt unchanged)
- `pnpm dist:mac` (or equivalent for the dev's OS) still produces an installable DMG
- Manual smoke 11 steps verified (the spec's smoke table filled in)
- `audit_event` has the 3 `inbound_questionnaire.*` event kinds after a smoke run
- v1 outbound flow regression test: an existing outbound questionnaire still generates answers correctly
