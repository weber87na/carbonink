...
2. **审计就绪功能集补全：通用证据附件 + 端到端数据血缘 + 版本历史**（Workiva 把
   audit-ready 定义成这张清单；Watershed "每个数字可追溯"、Normative "每条因子标
   注来源+版本"同向）
   - 现状：提取链路已有溯源（extraction→document、inbound_question→questionnaire），
     但**手工录入行无溯源**、没有"给任意记录挂佐证文档"的通用能力、没有
     EF→计算→报告的端到端血缘视图、版本历史无用户可见呈现。
   - 这是对 "audit-grade confidence" 产品承诺的直接补全——候选优先级最高。
...
4. **用户自带排放因子库**（Avarni 支持上传自购/自定义因子库；规模对标均为宣称：
   CCDB 30万+ / Normative 349k / Greenly 300k / Avarni 65k）
   - 自建库拼规模拼不过；开放导入（官方库、自购 ecoinvent、行业库）+ pinned
     snapshot 溯源是更合理的路线，与"因子来源+版本可见"的审计叙事互相加强。

5. **供应商问卷状态追踪 + 提醒，并把"零账户"明说成卖点**（Avarni/Persefoni 用免
   费供应商账户降门槛；Workiva 用平台内置问卷、无 xlsx 离线往返）
   - v2.0 inbound 已有 draft→sent→received→ingested 状态机，缺逾期提醒与批量状
     态视图；营销上把"供应商零账户、零平台注册"（vs 海外 SaaS 注册摩擦）讲出来。
