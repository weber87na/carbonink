...
## 3. ✅ 已完成（2026-05-26 → 05-27）— 用 `@earendil-works/pi-ai` 替换内部 LLM 客户端

**落地**：`llm-client.ts` 已被 `llm/ai-client.ts`（pi-ai 之上的 Effect 封装，
Tag/Layer 注入 + tagged AiErr）与 `run-ai.ts` boundary helper 取代；
`pi-catalog.ts` 动态供应商/模型目录进 Settings（含 DeepSeek 等 OpenAI 兼容
BYOK + 自定义 model id 逃生口）。spec/plan：本地
`2026-05-26-pi-ai-llm-client-replacement.md`，commits ~`b104193`。
（此节以下为当时的设计记录，保留存档。）
...
## 4. ✅ 已完成 v1（2026-05-27 → 05-28）— 用 `@earendil-works/pi-agent-core` 重写 answer-generation 流水线

**落地**：`llm/ai-agent.ts`（pi-agent-core `agentLoop` 之上的 Effect 封装：
submit_response 结构化收口、maxTurns/stalled/timeout 典型失败、AgentTrace）；
answer-generation 走 agent loop（5 个只读 inventory 工具 + 6 turns 预算 +
单发 fallback + `agent_answer.generate` 审计行）。spec/plan：本地
`2026-05-27-pi-agent-answer-generation.md`，commits `26d2fe3`..`7087f7f`。
2026-07-23 起 EF 匹配组级推荐复用同一封装（§8.1-①）。开放问题里的
traceroot / 思考过程侧栏仍留作后续。（此节以下为当时的设计记录，保留存档。）
...
- ~~完整 i18n key 迁移（v2.0 内嵌中文，未走 paraglide）~~ ✅ 已清账
  （2026-07-23，spec `2026-07-23-inbound-i18n-migration.md`：7 文件
  ≈130 处内嵌中文 → `inbound_*` keys，en 文案新撰、zh 逐字保真，
  迁移面非注释中文清零，现有测试零改动全绿）
...
1. ◐ **批量活动数据 → EF 自动映射 + 提取后异常检测**（Avarni 账单提取+百万行映射、
   ...
   ——四家跨市场在售，信号最强）
   - **v1 已完成（2026-07-21，commits `cba974a`..`b0d9b39`+）**：整表 xlsx/csv
     台账导入向导（/activities「批量导入」）——复用 EF 库导入 parser/别名映射/
     错误码脚手架；报告期向导级选择；源名称匹配现有 + 按组引导新建；EF **分组
     确认**（组 = 描述+单位+源，FTS+LLM 组级推荐 `recommendForText`，人拍板、
     组内批量应用，跨量纲无燃料绑定拒绝确认）；规则型异常检测四条（量纲错配、
     组内 >10× 中位数离群、文件内/库内疑似重复、期间错位）——警告不阻断、错误
     行跳过列报；导入单事务：台账原件内容寻址存档（doc_type='activity_import'）
     + 每行 evidence 回链 + pinned EF + `activity_data.bulk_imported` 审计事件；
     /activities 行加「台账导入」provenance 徽标。零 schema 迁移。测试
     1018 → 1063。spec/plan：本地 `2026-07-21-batch-activity-import.md`。
   - **live GUI e2e smoke 已补（2026-07-22）**：`tests/e2e/activity-import.spec.ts`
     ——真实 onboarding IPC 种库 → 向导全六步（真解析/真 FTS 候选/真入库）→
     台账导入徽标 ×4 → 溯源 drawer 见台账原件，6 张截图。注意 ABI 互斥：
     e2e 要 electron ABI（`pnpm build`），跑完 `rebuild:node` 才能回 vitest。
   - ✅ **多文件 PDF 批量提取队列已落地（2026-07-22，spec
     `2026-07-22-batch-extraction-queue.md`）**：/documents「批量识别 (N)」
     手动触发（LLM 花费按下即授权），BatchExtractionService 复用逐份
     classifyAndRun、并发 2、单批次、可取消（在途跑完即止）、失败聚合
     （classify_failed 留在未识别池天然可重试）；`extraction:batch-progress`
     推送全量快照 + `batch-status` 水合；确认粒度不变（逐份人审）。
     测试 1063 → 1077。
   - ✅ **组级 EF 推荐 agent 自纠错循环已落地（2026-07-23，spec
     `2026-07-23-agent-ef-match-loop.md`）**：`recommendForText` 升级为
     agent loop（复用 pi-agent-core 封装 `llm/ai-agent.ts`，方向 4 复用
     第一站）——`search_ef` 关键词重搜（CJK 单 token 用前缀查询命中）+
     `get_ef_detail` 读因子描述，4 turns 预算（`EF_MATCH_AGENT_MAX_TURNS`
     env 可调）；超限/异常/无 key 回落原 FTS+单发路径（从不 throw 契约
     不变）；产出仍是 MatcherResult、UI 零改动、「从不预选」零损耗；
     `ef_match.agent_trace` 审计事件（仅工具名序列/turn/token/时长，
     无 prompt 内容）。测试 1113 → 1120。
   - ✅ **离群倍数可配置（2026-07-23，spec
     `2026-07-23-import-outlier-threshold.md`）**：`import.outlier_ratio`
     每账套 setting（[2,1000]，默认 10，垃圾值回落）——四规则中唯一带数值
     阈值的一条；Settings → 组织资料「批量导入异常检测」子块；其余三条为
     二值判断不伪造配置面。测试 1139 → 1143。
   - **留作 v1.x/v2**：LLM 逐行匹配（本轮仍组级）。
   - 原始判断存档：AI 提取按单据走 5 stage；`ef-matcher-service` 匹配内核已复用；
     方向 4（pi-agent-core 化）天然装得下"匹配 → 校验 → 自我纠错"循环。
...
2. ✅ **审计就绪功能集补全：通用证据附件 + 端到端数据血缘 + 审计历史**（Workiva 把
   ...
   - **留作 v2**：版本 diff/回滚（本轮只做事件时间线）；~~evidence 附件计入
     报告导出~~（已被 2026-07-23 客户交付包覆盖——证据副本 + sha256 manifest
     随报告打包，见 §8.1-③）；发版前 live `pnpm dev` GUI smoke（本轮为
     vitest + 路由级集成验证）。

3. ◐ **顾问多客户工作流**（Avarni 已产品化：按客户计算规则保证逐年口径一致、客户只
   读交付报告、白标 logo/配色、按客户计价）
   - **v1 账套已落地（2026-07-22，spec `2026-07-22-client-workspaces.md`）**：
     一客户一独立 SQLite 文件（账套模式，brainstorm 否决单库多 org）——每库内部
     维持 singleton 组织不变式，零迁移零查询改动；`<userData>/workspaces.json`
     注册表（原子写、仅裸文件名防逃逸，首启自动登记 app.sqlite 为默认账套，
     存量用户无感）；切换 = 应答→cleanupIpc→closeAppDb→openAppDb+migrate→
     setupIpc（全 lazy 服务换新库）→renderer reload，新账套空库直接进
     onboarding（正是接新客户的流程）；Settings→账套 section（列表/当前徽标/
     新建/重命名/切换确认）；侧栏副标题在多账套时显示当前账套名。
     测试 1077 → 1093。
   - **后续切片**：✅ 删除账套（2026-07-22，归档重命名 .deleted-*，禁删
     active/最后一个）；✅ 导出报告白标 logo（2026-07-22，setting 按账套存 data URL，
     ISO+TCFD 封面渲染，512KB PNG/JPEG）；✅ 面向客户的只读交付包
     （2026-07-23，spec `2026-07-23-client-deliverable-bundle.md`：/reports
     「导出交付包 (ZIP)」= 报告 PDF + xlsx 附录 + `evidence/` 报告期证据
     副本（`<活动序号>-<原名>`，与附录活动表同序）+ `manifest.csv` sha256
     清单——审阅方免装碳墨离线校验；archiver 流式打包（exceljs 树内同版
     零新增依赖），证据缺失标 missing 不中断；ISO/TCFD 两 kind 通用；
     测试 1120 → 1128）；✅ 自动备份多账套感知 + 备份目标修正
     （2026-07-23，spec `2026-07-23-backup-workspace-aware.md`：修真 bug——
     DataBackupService 仍硬编码 `app.sqlite`，非默认账套 active 时手动导出
     /恢复/重置全部作用于错误文件；`connection.getAppDbPath()` 记录 live
     路径，三操作改为「当前账套」语义；自动备份逐账套
     `auto-backups/<id>/` 子目录、retention 7 与 23h due 均按账套、legacy
     平铺文件迁入默认账套流、非 active 库临时连接 checkpoint 后拷、单账套
     坏库不阻断；测试 1128 → 1139。同类清扫（spec
     `2026-07-23-mcp-workspace-aware.md`）：**MCP server 亦账套感知**——
     此前外部 agent 永远读 `app.sqlite`（= 默认账套 = 别的客户数据），
     现读 `workspaces.json` active 账套（缺注册表回落、basename 防逃逸、
     每次 tool call 现解析故切换即生效）；测试 1143 → 1149）。
   - 原始判断存档：CarbonInk 主用户就是多客户顾问。可做：按组织的计算规则预设、
     导出报告白标、面向客户的只读交付物。
...
5. ✅ **供应商问卷状态追踪 + 提醒，并把"零账户"明说成卖点**（Avarni/Persefoni 用免
   ...
   - **逾期提醒 v2 已完成（2026-07-13）**：启动时系统通知（每本地日至多一条聚合，
     ...
     1000 → 1018。
   - **营销收尾已完成（2026-07-21）**：首页 FAQ 新条目（en+zh，进 FAQPage
     JSON-LD）+ Differentiators 卡 3 扩写 + `llms.txt` 摘要/推荐场景，均讲出
     "供应商零账户、零平台注册、xlsx 邮件往返"；顺手把截图库 en 标题
     "Fill supplier surveys" 纠正为 "Fill customer questionnaires"（避免与
     inbound 叙事撞词）。spec：本地 `specs/2026-07-21-zero-account-marketing.md`。
     留作 follow-up：供应商披露页截图素材（重跑 e2e tour-05b）、"如何向供应商
     收数据" SEO 指南文章、部署（wrangler，见 cloud/DEPLOY.md）。

6. ◐ **AI 生成披露报告**（碳阻迹碳云已产品化生成符合 CDP/ESG 框架的图文报告）
   - **v1 TCFD 四支柱已落地（2026-07-22，spec `2026-07-22-tcfd-report.md`）**：
     /reports 报告类型切换（ISO 14064-1 | TCFD）；`tcfd-narrative.ts` 治理/
     战略/风险管理/指标与目标四段叙事（沿用 ISO 叙事全部护栏 + TCFD 特有
     「无数据不编造情景」护栏，数字只能来自 assembleReportData）；预览含
     指标附表（Scope 合计、主要排放源、基准年/上期对比，全部查表不走 LLM）；
     `report:generate-tcfd`/`export-tcfd-pdf` 复用共享 inflight + cancel +
     progress 通道；print-render 新 kind 'tcfd_report' 走同一 PDF 管线。
     结构稳定不随年变（对比 CDP 年年改版），A 股 ESG 气候章节同样适用。
     测试 1093 → 1104。
   - **后续切片**：✅ TCFD 叙事可编辑（2026-07-22）；✅ xlsx 附录（2026-07-22，
     四支柱叙事页 + 同款数据附表，导出合并为 PDF+Excel）；CDP 题号映射
     （等需求验证）；情景分析等高级披露（等数据源）。
   - 原始判断存档：`report-narrative.ts` 已有叙事生成雏形，可扩展为按披露
     框架结构化输出。
