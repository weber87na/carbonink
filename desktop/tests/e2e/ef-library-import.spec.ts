...
test('EF library import: pick → preview → import → replace → delete', async () => {
  ...
  try {
    ...
    await window.getByText('ef-library-import.csv').waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await window.getByText(/已导入 3 条因子|imported 3 factors/i).waitFor({ timeout: 10_000 });
