...
describe('extractHint', () => {
  it('china_utility.v1 → supplier_name', () => {
    expect(extractHint('china_utility.v1', { supplier_name: '国家电网北京' })).toContain(
      '国家电网北京',
    );
  });
  ...
  it('freight.v1 → mode + vehicle_class + supplier_name', () => {
    const out = extractHint('freight.v1', {
      mode: 'road',
      vehicle_class: '重型卡车',
      supplier_name: '顺丰',
    });
  ...
  it('purchase.v1 → category + item_description + supplier_name', () => {
    const out = extractHint('purchase.v1', {
  ...
  it('travel.v1 → mode + travel_class + supplier_name', () => {
    const out = extractHint('travel.v1', {
      mode: 'air',
      travel_class: '经济舱',
      supplier_name: '国航',
    });
