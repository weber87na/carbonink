...
describe('LLMClient.recommendEfs', () => {
  it('builds a structured-output call with the recommendation schema', async () => {
    const fakeConfig: ProviderConfig = { provider: 'openai', model: 'gpt-4o-mini', apiKeyKeyref: 'llm.openai.apikey' };
    const client = new LLMClient({ credentials: { get: () => 'sk-fake' } as any });
