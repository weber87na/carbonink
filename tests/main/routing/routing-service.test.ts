...
function setup() {
  ...
  runMigrations(db);
  const testLayer = Layer.mergeAll(Layer.succeed(DbTag, db), Layer.succeed(AmapKeyTag, 'fake-key'));
  return { db, testLayer };
}
...
describe('RoutingService.lookup', () => {
  it('air mode → haversine; second call returns cached:true', async () => {
    ...
    const result1 = await Effect.runPromise(
      routingSvc
        .lookup({ mode: 'air', origin: 'PEK', destination: 'JFK' })
  ...
  it('driving mode calls AMap client; result cached', async () => {
    ...
    const result = await Effect.runPromise(
      routingSvc
        .lookup({ mode: 'driving', origin: '北京', destination: '上海' })
        .pipe(Effect.provide(testLayer)),
    );
    ...
    const cached = db.prepare(`SELECT distance_km FROM routing_cache`).get() as {
      distance_km: number;
    };
