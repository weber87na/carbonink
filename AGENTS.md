...
- **Git: commit freely without asking; push only when explicitly allowed.**
  ...
  first.
- **Tests must not regress.** `pnpm desktop:test` is at **1255/1255** (2026-08-13,
  after the agent bridge + inventory readiness checks landed). Don't land a
  commit that drops the count.
